...
# Kalkulatu eta plot egin FFT aukeratutako datuetarako (logaritmik graduan)
plt.subplot(2, 2, 4)  # Hiru errenkadak, bi zutabe, hirugarren subplota (logaritmik graduan)

# FFT emaitza bera erabiliz, baina logaritmik graduan erakutsi
...
# Irudiak gordetzeko
irudiak_gordetzeko_bidea = os.path.join(args.folder_path, folder_name + '.png')
plt.savefig(irudiak_gordetzeko_bidea)

# Aukeratutako datuak CSV fitxategi gisa gorde
aukeratutako_datuak = df.iloc[start_index:end_index]
csv_gordetze_bidea = os.path.join(args.folder_path, folder_name + '.csv')
aukeratutako_datuak.to_csv(csv_gordetze_bidea, index=False)
...
irudiak_gordetzeko_karpeta_izena = f"{uneko_data}_PNG"

# Irudiak gordeko diren karpeta-muturra
irudiak_gordetzeko_karpeta_bidea = os.path.join(os.getcwd(), irudiak_gordetzeko_karpeta_izena)

# Egiaztatu karpeta existitzen dela, eta ez badago, sortu
if not os.path.exists(irudiak_gordetzeko_karpeta_bidea):
    os.makedirs(irudiak_gordetzeko_karpeta_bidea)

# Irudiaren bidea irudiak_gordetzeko_karpeta_bidean
irudiak_gordetzeko_karpeta_bidea_une_honetan = os.path.join(irudiak_gordetzeko_karpeta_bidea, f'{folder_name}_{args.start_time}s_etatik_{args.end_time}s_6_programa.png')

# Irudia kopiatu irudiak_gordetzeko_karpeta_bidean
shutil.copy(irudiak_gordetzeko_bidea, irudiak_gordetzeko_karpeta_bidea_une_honetan)
