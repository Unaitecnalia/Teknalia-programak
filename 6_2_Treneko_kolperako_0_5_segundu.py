...
# Plot egin FFT-arekin kalkulatutako max_value-arekin eta erabilgarritasun egokitutako y-ardatzaren mugak (lineal graduan)
...
# FFT emaitzak erabiliz baina plot egin logaritmikoan
...
plt.title('Aukeratutako Datuen FFT-a Logaritmikoan (X, Y, eta Z)')
plt.grid(True)
plt.legend()
...
une_honetako_data = datetime.datetime.now().strftime("%Y%m%d")

# Une honetako karpeta-izena erabiliz
irudiak_karpeta_izena = f"{une_honetako_data}_PNG"

# Irudiak gordeko diren karpeta-muturra
irudiak_karpeta_bidea = os.path.join(os.getcwd(), irudiak_karpeta_izena)

# Egiaztatu karpeta existitzen dela, eta ez badago, sortu
if not os.path.exists(irudiak_karpeta_bidea):
    os.makedirs(irudiak_karpeta_bidea)

# Irudiaren bidea irudiak_karpeta_bidean
irudiak_bidean_une_honetan = os.path.join(irudiak_karpeta_bidea, f'{folder_name}_{args.start_time}s_etatik_{args.end_time}s_6_programa.png')

# Irudia kopiatu irudiak_karpeta_bidean
shutil.copy(irudiaren_bidea, irudiak_bidean_une_honetan)
